...
// Same configuration as in the coalesced version.
constexpr int THREADS_PER_BLOCK   = 256;
constexpr int NUM_BLOCKS          = 512;   // 512 * 256 = 131072 threads
constexpr int ELEMENTS_PER_THREAD = 256;
...
//   [NUM_THREADS_TOTAL][ELEMENTS_PER_THREAD] in row-major order.
// For a fixed iteration i, threads in the same warp access
//   input[tid * elements_per_thread + i],
// which are far apart in memory (large stride) -> poor coalescing.
// ---------------------------------------------------------------------------
__global__ void uncoalesced_kernel(const float* __restrict__ input,
                                   ...
                                   int num_threads_total)
{
    ...
    for (int i = 0; i < elements_per_thread; ++i) {
        ...
        // input[tid * elements_per_thread + i],
        // which are spaced by 'elements_per_thread' in memory.
...
int main() {
    std::cout << "Running uncoalesced_kernel\n";
    ...
    // Warm-up.
    uncoalesced_kernel<<<grid, block>>>(d_input, d_output,
